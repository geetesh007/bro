...
class Sitemap extends Controller {
    public function process() {
        ...
        for ($ipost=1; $ipost <=ceil($Posts/SITEMAP_PAGE); $ipost++) { 
            echo '<sitemap>';
                echo '<loc>'.APP.'/sitemap.posts_'.$ipost.'.xml</loc>';
                echo '<lastmod>'.date("Y-m-d")."T".date("H:i:s").'+03:00</lastmod>';
            echo '</sitemap>';
        }
        // episodes
        for ($iepisode=1; $iepisode <=ceil($Episodes/SITEMAP_PAGE); $iepisode++) { 
            echo '<sitemap>';
                echo '<loc>'.APP.'/sitemap.episodes_'.$iepisode.'.xml</loc>';
                echo '<lastmod>'.date("Y-m-d")."T".date("H:i:s").'+03:00</lastmod>';
            echo '</sitemap>';
        }
        ...
        for ($iactor=1; $iactor <=ceil($Actors/SITEMAP_PAGE); $iactor++) { 
            echo '<sitemap>';
                echo '<loc>'.APP.'/sitemap.actors_'.$iactor.'.xml</loc>';
                echo '<lastmod>'.date("Y-m-d")."T".date("H:i:s").'+03:00</lastmod>';
            echo '</sitemap>';
        }
        // discussions
        ...
        for ($icategories=1; $icategories <=ceil($Categories/SITEMAP_PAGE); $icategories++) { 
            echo '<sitemap>';
                echo '<loc>'.APP.'/sitemap.categories_'.$icategories.'.xml</loc>';
                echo '<lastmod>'.date("Y-m-d")."T".date("H:i:s").'+03:00</lastmod>';
            echo '</sitemap>';
        }
        
        // categories
        for ($iusers=1; $iusers <=ceil($Users/SITEMAP_PAGE); $iusers++) { 
            echo '<sitemap>';
                echo '<loc>'.APP.'/sitemap.users_'.$iusers.'.xml</loc>';
                echo '<lastmod>'.date("Y-m-d")."T".date("H:i:s").'+03:00</lastmod>';
            echo '</sitemap>';
        }
        echo '</sitemapindex>'; 
